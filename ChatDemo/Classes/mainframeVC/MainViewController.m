...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(mainNotificationChangeBackground:) name:Notification_main  object:nil];
    ...
    UIImage *image = [[UIImage alloc] initWithContentsOfFile:MainImg_path];
    if (image) {
...
- (void)mainNotificationChangeBackground:(NSNotification*)noti {
    UIImage *image = [[UIImage alloc] initWithContentsOfFile:MainImg_path];
    if (image) {
//        UIImage *newImage = [UIImage wechatScaleImg:image];
